...
void Model::setupMesh(Shader* shader, int i)
{
	_meshes[i].setup(shader);
}
...
int Model::getMeshIndicesSize(int n)
{
	return _meshes[n].indices.size();
}

Mesh * Model::getMesh(int n)
{
	return &_meshes[n];
}
...
void Model::loadModel(std::string path) {
	...
	std::cout << "directory variable : " << _directory << std::endl;
...
std::vector<Texture> Model::loadTextures(aiMaterial * material, aiTextureType type, std::string typeName)
{
	...
	for (int i = 0; i < material->GetTextureCount(type); i++) {
		...
		for (GLuint o = 0; o < _texturesLoaded.size(); o++) {
			if (std::strcmp(str.C_Str(), _texturesLoaded[o].path.C_Str()) == 0) {
				textures.push_back(_texturesLoaded[o]);
				skip = true;
				break;
			}
		}
		if (!skip) {
			Texture texture;
			texture.id = utilities::TextureFromFile(str.C_Str(), this->_directory);
			std::cout << "directory : " << _directory << " fileName : " << str.C_Str() << std::endl;
